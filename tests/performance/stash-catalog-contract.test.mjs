...
const catalog = JSON.parse(await readFile(new URL("../../assets/library/catalogs/stash-catalog.json", import.meta.url), "utf8"));
...
test("Stash prefers the compact catalog and preserves the full-index fallback", () => {
  assert.match(serviceSource, /const STASH_CATALOG_BUNDLE_PATH = "assets\/library\/catalogs\/stash-catalog\.json"/);
