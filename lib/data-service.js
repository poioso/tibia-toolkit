...
async function getBoostedForUi() {
  const remote = await fetchGameDataHubJson(
    `${getGameDataHubBase()}/api/game/boosted`,
    "Falha ao consultar os destaques do dia",
    GAME_DATA_HUB_BOSS_TIMEOUT_MS
  );

  return {
    creature: remote?.creature?.current || null,
    boss: remote?.boss?.current || null,
