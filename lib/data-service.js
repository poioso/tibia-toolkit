...
const BOOSTED_PUBLIC_API_ENDPOINT = "https://tibiatoolkit.com/api/boosted";
...
async function getBoostedForUi() {
  const response = await fetchWithTimeout(BOOSTED_PUBLIC_API_ENDPOINT, GAME_DATA_HUB_BOSS_TIMEOUT_MS);
  const body = await response.text();
  if (!response.ok) {
    throw new Error(`Falha ao consultar os destaques do dia (${response.status}): ${body}`);
  }
  const remote = JSON.parse(body);

  return {
    creature: remote?.creature || null,
    boss: remote?.boss || null,
...
async function getBooksDocumentsCatalog() {
  if (!booksDocumentsCatalogPromise) {
    booksDocumentsCatalogPromise = dataServiceRuntime.readJsonAsset(BOOKS_DOCUMENTS_AUDIT_PATH)
      .then((bundle) => (Array.isArray(bundle?.records) ? bundle.records : []).map((record) => {
...
async function getBooksDocuments(payload = {}) {
  ...
  if (slug) {
    return { detail: catalog.find((entry) => entry.slug === slug) || null };
  }
  ...
  const sorted = [...filtered].sort((left, right) => {
    ...
    if (sort === "name-desc") return -name || entryAuthor;
