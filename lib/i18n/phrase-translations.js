...
const OFFICIAL_TIBIA_WORLD_NAMES = [
  ...
  "Ignitera", "Inabra", "Issobra", "Jadebra", "Junera", "Kalanta", "Kalibra", "Kalimera",
  "Kanda", "Karmeya", "Lobera", "Luminera", "Lutabra", "Luzibra", "Maligna", "Menera",
  "Monstera", "Monza", "Mystera", "Nefera", "Nevia", "Noctalia", "Oceanis", "Ombra",
  "Opulera", "Ourobra", "Pacera", "Peloria", "Penumbra", "Premia", "Quelibra", "Quidera",
  "Quintera", "Rasteibra", "Refugia", "Retalia", "Secura", "Serdebra", "Solidera", "Sombra",
  "Sonira", "Stralis", "Talera", "Tempestera", "Terribra", "Thyria", "Tornabra", "Unebra",
  "Ustebra", "Venebra", "Victoris", "Vunira", "Wintera", "Xybra", "Xyla", "Xymera",
  "Yonabra", "Yovera", "Yubra", "Zuna", "Zunera"
];
const protectedPhraseValues = new Set(OFFICIAL_TIBIA_WORLD_NAMES);

function escapeProtectedPhrase(value) {
  return String(value || "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function protectRegisteredPhrases(value) {
  let text = String(value ?? "");
  const replacements = [];
  const values = [...protectedPhraseValues]
    .filter((entry) => entry && text.includes(entry))
    .sort((left, right) => right.length - left.length);

  values.forEach((entry) => {
    const token = `__TIBIA_PROPER_NOUN_${replacements.length}__`;
    const pattern = new RegExp(`(?<![\\p{L}\\p{N}])${escapeProtectedPhrase(entry)}(?![\\p{L}\\p{N}])`, "gu");
    if (!pattern.test(text)) return;
    pattern.lastIndex = 0;
    text = text.replace(pattern, token);
    replacements.push([token, entry]);
  });

  return { text, replacements };
}

function restoreRegisteredPhrases(value, replacements) {
  return replacements.reduce(
    (current, [token, original]) => String(current).replaceAll(token, original),
    String(value ?? "")
  );
}

export function registerProtectedPhrases(values = []) {
  for (const value of Array.isArray(values) ? values : [values]) {
    const normalized = String(value?.name || value || "").trim();
    if (normalized) protectedPhraseValues.add(normalized);
  }
}
...
export function translatePhraseSync(locale, value, phraseMap = {}) {
  ...
  const protection = protectRegisteredPhrases(source);
  const protectedSource = protection.text;
  const restore = (nextValue) => restoreRegisteredPhrases(nextValue, protection.replacements);
  const direct = phraseMap[normalizePhraseKey(protectedSource)] ?? phraseMap[protectedSource];

  if (direct && direct !== protectedSource) {
    return restore(direct);
  }

  const rules = PHRASE_RULES[normalizeLocale(locale)] || [];
  for (const [pattern, replacer] of rules) {
    pattern.lastIndex = 0;
    if (!pattern.test(protectedSource)) {
      continue;
    }

    const nextText = typeof replacer === "function"
      ? protectedSource.replace(pattern, (...args) => replacer(...args.slice(0, -2)))
      : protectedSource.replace(pattern, replacer);

    if (nextText && nextText !== protectedSource) {
      return restore(nextText);
    }
  }
  ...
  if (segments.length > 1) {
    ...
    const translatedSegments = segments.map((segment) => {
      ...
      const translated = translatePhraseSync(locale, trimmed, phraseMap);
      if (!translated || translated === trimmed) return segment;
      changed = true;
      return segment.replace(trimmed, translated);
    });

    if (changed) {
      return restore(translatedSegments.join(""));
    }
  }

  return restore(direct || protectedSource);
}
