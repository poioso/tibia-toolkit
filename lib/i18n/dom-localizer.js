...
const I18N_PRESERVE_SELECTOR = "[data-i18n-preserve],[translate='no']";
...
function setLocalizedMarkup(element, value) {
  const parsedDocument = new DOMParser().parseFromString(String(value || ""), "text/html");
  parsedDocument.body.querySelectorAll("script,iframe,object,embed,link,meta,base,form").forEach((node) => node.remove());
  ...
  element.replaceChildren(
    ...[...parsedDocument.body.childNodes].map((node) => document.importNode(node, true))
  );
}
...
function shouldSkipTextNode(node) {
  ...
  return Boolean(
    node.parentElement.closest(
      `[data-i18n],[data-i18n-html],${I18N_PRESERVE_SELECTOR},svg,picture,video,audio,canvas,iframe,object,embed`
    )
  );
}
...
function shouldSkipPhraseAttributes(element) {
  return Boolean(
    element?.matches?.(`img,svg,picture,video,audio,source,track,canvas,iframe,object,embed,${I18N_PRESERVE_SELECTOR}`)
    || element?.closest?.(I18N_PRESERVE_SELECTOR)
  );
}
...
function applyKeyedTranslations(root) {
  ...
  keyedElements.forEach((element) => {
    ...
    if (htmlKey) {
      setLocalizedMarkup(element, t(htmlKey));
    } else if (textKey) {
