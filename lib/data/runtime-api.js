import { getAppLocale } from "../i18n/app-i18n.js";
import { loadPhraseTranslationMap, translatePhraseSync } from "../i18n/phrase-translations.js";
...
export async function fetchStashItemPreview(payload) {
  return localizeRuntimePayload("fetch-stash-item-preview", sendMessage({ type: "fetch-stash-item-preview", payload }));
}
...
export async function fetchBooksDocuments(payload = {}) {
  return localizeRuntimePayload("fetch-books-documents", sendMessage({ type: "fetch-books-documents", payload }));
}
...
export async function fetchStashMarketRefreshStatus() {
  return sendMessage({ type: "get-stash-market-refresh-status" });
}

export async function reserveStashMarketRefresh() {
  return sendMessage({ type: "reserve-stash-market-refresh" });
}
...
export async function fetchMiniWorldChanges(payload) {
  return sendMessage({ type: "fetch-mini-world-changes", payload });
}

export async function fetchBoosted() {
  return sendMessage({ type: "fetch-boosted" });
}
