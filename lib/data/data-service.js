...
const ITEM_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/item-audit-corrections.json";
const ITEM_LOOT_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/item-loot-audit-corrections.json";
const ITEM_BUNDLE_REVISIONS_PATH = "assets/data/item-bundle-revisions.json";
const WEEKLY_TASK_ITEMS_BUNDLE_PATH = "assets/data/weekly-task-items.json";
const ITEM_SPRITE_ATLAS_BUNDLE_PATH = "assets/data/item-sprite-atlas.json";
const STASH_CATALOG_BUNDLE_PATH = "assets/data/stash-catalog.json";
...
const NPC_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/npc-audit-corrections.json";
...
const CREATURE_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/creature-audit-corrections.json";
const CREATURE_LOOT_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/creature-loot-audit-corrections.json";
const BOSS_TABLE_AUDIT_CORRECTIONS_BUNDLE_PATH = "assets/data/boss-table-audit-corrections.json";
const LIBRARY_SPRITE_PATHS_BUNDLE_PATH = "assets/data/library-sprite-paths.json";
const BOOKS_DOCUMENTS_AUDIT_PATH = "assets/data/books-documents/tibiawiki.audit.json";
...
const SITE_LIBRARY_CANONICAL_PATH = "assets/data/site-library-canonical.json";
const SITE_LIBRARY_CHUNKS_MANIFEST_PATH = "assets/data/site-library-chunks/manifest.json";
const SITE_LIBRARY_CHUNKS_ROOT = "assets/data/site-library-chunks";
...
const NPC_IMAGE_FALLBACKS = {
  hireling: "assets/ui/npcs/Hireling_(Trader).gif",
  "wes the blacksmith": "assets/ui/npcs/Wes_The_Blacksmith.gif",
  'hireling "trader"': "assets/ui/npcs/Hireling_(Trader).gif",
  "hireling trader": "assets/ui/npcs/Hireling_(Trader).gif"
};
...
async function fetchOfficialTibiaCharacterJson(name) {
  const response = await fetchWithTimeout(
    ...
    {
      Accept: "text/html",
      "User-Agent": "Mozilla/5.0 (compatible; TibiaToolkit/0.7.0)"
    }
  );
...
function decodeOfficialTibiaHtmlText(value) {
  return String(value || "")
    .replace(/<[^>]+>/g, " ")
    .replace(/&(?:#(\d+)|#x([0-9a-f]+)|nbsp|amp|quot|apos|lt|gt);/gi, (entity, decimal, hexadecimal) => {
      if (decimal) {
...
async function getMiniWorldChangesForUi(payload = {}) {
  const [catalogBundle, localeBundle] = await Promise.all([
    dataServiceRuntime.readJsonAsset("assets/data/mini-world-changes/catalog.json"),
    dataServiceRuntime.readJsonAsset("assets/i18n/mini-world-changes.json").catch(() => ({}))
  ]);
...
async function getStashItemPreview(payload) {
  ...
  return {
    item,
    selectedWorld: {
      name: itemNameFromSlug(worldSlug),
      slug: worldSlug,
      last_update: null,
      tc_price: null
    },
...
function canonicalLootImageUrl(entry = {}) {
  ...
  if (bySlug) return canonicalLibraryMediaUrl(image, dataServiceRuntime.getAssetUrl(`assets/data/items/by-slug/${decodeURIComponent(bySlug)}`));
    ...
    options: (Array.isArray(entry?.options) ? entry.options : []).map((option) => ({
      ...
      images: (Array.isArray(option?.images) ? option.images : []).map((image) => {
        ...
        return downloadedMedia
          ...
          : filename
            ? { ...image, src: dataServiceRuntime.getAssetUrl(`assets/data/item-proficiency-icons/${filename}`) }
            : image;
      })
    }))
  }));
...
async function applyCanonicalItemDocument(target) {
  ...
  target.notes_image = canonicalLibraryMediaUrl(noteImagePath, noteImageName
    ? dataServiceRuntime.getAssetUrl(`assets/data/items/notes/${noteImageName}`)
    : "");
...
function normalizeBookImage(source) {
  ...
  const filename = remoteFilename
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
  return dataServiceRuntime.getAssetUrl(`assets/data/books-documents/images/${filename || "book"}${extension}`);
}
...
function getLocalLibrarySpriteUrl(kind, name, slugHint = "") {
  ...
  return extension ? dataServiceRuntime.getAssetUrl(`assets/data/${kind}/${slug}.${extension}`) : "";
}
...
function getLocalLibrarySpriteStillUrl(kind, name, slugHint = "") {
  ...
  return ["gif", "png", "webp"].includes(extension.toLowerCase())
    ? dataServiceRuntime.getAssetUrl(`assets/data/library-thumbnails/${kind}/${slug}.png`)
    : dataServiceRuntime.getAssetUrl(`assets/data/${kind}/${slug}.${extension}`);
}
...
async function readStoredWorldMarketSnapshot(worldSlug) {
  const cacheKey = `market-world:${slugifyWorldName(worldSlug || DEFAULT_WORLD)}`;
  const cachedEntry = await getCacheEntry(cacheKey);
  const cachedValue = normalizeCachedWorldMarketValue(cachedEntry?.value);
  ...
  const prefix = `market-values:${slugifyWorldName(worldSlug || DEFAULT_WORLD)}:`;
...
async function buildCanonicalItemRecord(document = {}, legacyItem = null) {
  ...
  return {
    ...
    notes_image: (() => {
      ...
      return canonicalLibraryMediaUrl(noteImagePath, filename ? dataServiceRuntime.getAssetUrl(`assets/data/items/notes/${filename}`) : "");
    })(),
...
function getItemImageUrl(itemMeta) {
  ...
  if (directLocalImage.startsWith("/library/items/")) {
    return dataServiceRuntime.getAssetUrl(directLocalImage.replace(/^\/library\/items\//, "assets/data/items/"));
  }
  ...
  if (slug) {
    ...
    return dataServiceRuntime.getAssetUrl(`assets/data/items/by-slug/${slug}.${extension}`);
  }
...
function getNpcImageUrl(npcName) {
  ...
  if (normalizedNpcName.includes("hireling")) {
    return dataServiceRuntime.getAssetUrl("assets/ui/npcs/Hireling_(Trader).gif");
  }

  if (normalizedNpcName.includes("wes") && normalizedNpcName.includes("blacksmith")) {
    return dataServiceRuntime.getAssetUrl("assets/ui/npcs/Wes_The_Blacksmith.gif");
  }
