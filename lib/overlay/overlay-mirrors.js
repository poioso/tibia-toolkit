...
export const MIRROR_SOURCE_GAMES = new Set(["tibia", "rubinot", "medivia"]);

export function normalizeMirrorSourceGame(value) {
  const sourceGame = typeof value === "string" ? value.trim().toLowerCase() : "";
  return MIRROR_SOURCE_GAMES.has(sourceGame) ? sourceGame : "tibia";
}
...
export function normalizeOverlayMirrorEntry(rawEntry) {
  ...
  const sourceGame = Object.prototype.hasOwnProperty.call(rawEntry, "sourceGame")
    ? normalizeMirrorSourceGame(rawEntry.sourceGame)
    : null;
  ...
  return {
    ...
    displayBounds,
    sourceBounds,
    sourceType: rawEntry.sourceType === "obs-window" ? "obs-window" : "tibia",
    // Existing profiles have no sourceGame. Runtime treats that as Tibia, but
    // keeping the field absent prevents a normal save from migrating/rewrite
    // their profile data merely because this feature was introduced.
    ...(sourceGame ? { sourceGame } : {}),
    sourceHwnd: Number.isFinite(Number(rawEntry.sourceHwnd)) ? Number(rawEntry.sourceHwnd) : 0,
    sourceCaptureId: typeof rawEntry.sourceCaptureId === "string" ? rawEntry.sourceCaptureId.trim() : "",
