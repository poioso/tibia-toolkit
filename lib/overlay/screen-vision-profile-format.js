...
export function mirrorProfileToOverlayState(profileJson, audioJson, options = {}) {
  ...
  // The established profile format predates multi-client Mirror and therefore
  // stores no per-region game identifier. The selected profile namespace is
  // the authoritative source when restoring its regular regions.
  const sourceGame = ["rubinot", "medivia"].includes(String(options.sourceGame || "").trim().toLowerCase())
    ? String(options.sourceGame).trim().toLowerCase()
    : "tibia";
...
function mapOverlayMirrorToProfileRegion(region, index) {
  ...
  return {
    ...
    IsObsMirror: region.sourceType === "obs-window",
    IsFixedCrop: Boolean(region.isFixedCrop),
    AllowSnapping: region.allowSnapping !== false,
    ObsWindowTitle: region.sourceType === "obs-window" ? String(region.sourceWindowTitle || "").trim() : null,
    ObsProcessName: region.sourceType === "obs-window" ? String(region.sourceProcessName || "").trim() : null,
    ObsWindowHandle: region.sourceType === "obs-window" ? Number(region.sourceHwnd) || 0 : 0,
    ObsCaptureSourceId: region.sourceType === "obs-window" ? String(region.sourceCaptureId || "").trim() : "",
    SourceBoundsX: Number(region.sourceBounds?.x) || 0,
    SourceBoundsY: Number(region.sourceBounds?.y) || 0,
    SourceBoundsWidth: Number(region.sourceBounds?.width) || captureBounds.width,
    SourceBoundsHeight: Number(region.sourceBounds?.height) || captureBounds.height,
    OriginalCropX: 0,
    OriginalCropY: 0,
    OriginalCropWidth: 0,
...
function mapProfileRegionToOverlayMirror(region, index, options = {}) {
  ...
  const obsSourceBounds = region.IsObsMirror
    ? normalizeBounds({
        x: region.SourceBoundsX,
        y: region.SourceBoundsY,
        width: region.SourceBoundsWidth,
        height: region.SourceBoundsHeight
      }, { width: captureBounds.width, height: captureBounds.height })
    : null;
  const sourceBounds = obsSourceBounds || (options.sourceBounds
  ...
  return createOverlayMirrorEntry({
    ...
    sourceBounds,
    sourceType: region.IsObsMirror ? "obs-window" : "tibia",
    sourceGame: options.sourceGame,
    sourceHwnd: region.IsObsMirror ? Number(region.ObsWindowHandle) || 0 : 0,
    sourceCaptureId: region.IsObsMirror ? String(region.ObsCaptureSourceId || "").trim() : "",
    sourceWindowTitle: String(region.IsObsMirror ? region.ObsWindowTitle : options.sourceTitle || "").trim(),
    sourceProcessName: String(region.IsObsMirror ? region.ObsProcessName : options.sourceProcessName || "").trim(),
    relativeBounds: {
...
function mapOverlayTimerToProfileTimer(timer, index) {
  ...
  return {
    ...
    Duration: clampInteger(timer.durationSeconds, 1, 86400, 60),
    Volume: timerVolume,
    ...
    StartTime: null,
    EndTime: null,
    TimerKind: String(timer.timerKind || "standard"),
    PresetId: String(timer.presetId || ""),
    ClockMode: String(timer.clockMode || "runtime"),
    PersistentEndsAtMs: Number.isFinite(Number(timer.persistentEndsAtMs)) ? Number(timer.persistentEndsAtMs) : null
  };
}
...
function mapProfileTimerToOverlayTimer(timer, index) {
  ...
  return createOverlayTimerEntryFromDraft({
    name: String(timer.Name || `Timer ${index + 1}`).trim(),
    durationSeconds: clampInteger(timer.Duration, 1, 86400, 60),
    ...
    persistentEndsAtMs: timer.PersistentEndsAtMs,
    hotkey: {
