...
export const TIMER_DURATION_MAX_SECONDS = 60 * 60 * 24;
...
export function normalizeOverlayTimerDraft(rawDraft) {
  ...
  return {
    ...
    timerKind: normalizeTimerKind(source.timerKind),
    presetId: normalizePresetId(source.presetId),
    clockMode: normalizeTimerClockMode(source.clockMode),
    persistentEndsAtMs: normalizePersistentEndsAtMs(source.persistentEndsAtMs)
  };
}
...
export function normalizeOverlayTimerEntry(rawTimer) {
  ...
  return {
    ...
    timerKind: normalizeTimerKind(rawTimer.timerKind),
    presetId: normalizePresetId(rawTimer.presetId),
    clockMode: normalizeTimerClockMode(rawTimer.clockMode),
    persistentEndsAtMs: normalizePersistentEndsAtMs(rawTimer.persistentEndsAtMs),
...
function normalizeTimerKind(value) {
  ...
  return ["standard", "spell", "potion", "food"].includes(key) ? key : "standard";
}

function normalizePresetId(value) {
  const text = typeof value === "string" ? value.trim().toLowerCase() : "";
  return text.replace(/[^a-z0-9-]/g, "").slice(0, 100);
}

function normalizeTimerClockMode(value) {
  return String(value || "").trim().toLowerCase() === "wall-clock" ? "wall-clock" : "runtime";
}

function normalizePersistentEndsAtMs(value) {
